...
from pipeline_functions import (  # type: ignore
    create_lists,
    networkx_graph_from_lists,
    transform_node_tuple_to_two_fields,
    transform_edge_tuple_to_three_fields,
)
...
def pipeline(
    ...
    graph = networkx_graph_from_lists(
        nodes_iterable=transform_node_tuple_to_two_fields(nodes_list),
        edges_iterable=transform_edge_tuple_to_three_fields(edges_list),
...
if __name__ == "__main__":
    ...
    # MODIFY THIS: Choose between 1,2,3. By default value 1 will be taken.
    option_profiler = 3
    ...
    file_path_logs = create_log_filepath(
        FILE_PATH_LOGS, filename_nodes, ending_filename="lists"
    )
    ...
    logger.info("\tNumber of nodes(list): {}".format(len(nodes_ds)))
    logger.info("\tNumber of edges(list): {}".format(len(edges_ds)))
